...
Immune escape happens when new variants are introduced.  One new variant is introduced at a uniform random time during each 3-month interval.  The immune escape of each successive variant compared to the last variant is calculated as $1 - (1 - IE)^{0.25}$, where $IE$ is the annual immune escape specified by the scenario. This is reduction is applied to both infection- and vaccine-induced immunity.
